...
# Database: gim
# Generation Time: 2018-08-14 01:41:53 +0000
# ************************************************************
...
CREATE TABLE `im_device` (
  ...
  `user_agent` varchar(120) DEFAULT '' COMMENT '设备描述',
...
CREATE TABLE `im_user` (
  ...
  `mail` varchar(32) CHARACTER SET utf8mb4 NOT NULL DEFAULT '' COMMENT '邮箱',
  `verify_m` tinyint(1) NOT NULL DEFAULT '0',
  PRIMARY KEY (`id`),
...
# Dump of table im_verify_code
# ------------------------------------------------------------

DROP TABLE IF EXISTS `im_verify_code`;

CREATE TABLE `im_verify_code` (
  `uuid` varchar(44) NOT NULL DEFAULT '',
  `verify` varchar(12) NOT NULL DEFAULT '',
  `vld` int(12) NOT NULL DEFAULT '0' COMMENT '有效期至，0表示不会失效',
  `v_type` tinyint(1) NOT NULL DEFAULT '0' COMMENT '0 mail,1 MSM',
  `user_id` varchar(44) NOT NULL DEFAULT '',
  `status` tinyint(1) NOT NULL DEFAULT '0' COMMENT '验证状态',
